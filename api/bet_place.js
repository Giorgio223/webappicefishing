...
export default async function handler(req, res) {
  try {
    ...
    const pendingKey = `bet:pending:${userWallet}`;     // set of roundIds

    // легкая блокировка на пользователя
    ...
    if (cur < amountNano) return res.status(400).json({ error:"insufficient" });
    ...
    // помечаем round pending (чтобы settlement знал что есть что считать)
